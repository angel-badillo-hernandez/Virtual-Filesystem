    ...
    if "--help" in flags:
        ...
        try:
            if len(params)>1:
                ...
                # mode = os.stat(params).st_mode
                if "r" in permissions:
                    mode |= 0o400  # Add read permission (user)
                    mode |= 0o040  # Add read permission (group)
                    mode |= 0o004  # Add read permission (others)
                    
                if "w" in permissions:
                    mode |= 0o200  # Add write permission (user)
                    mode |= 0o020  # Add write permission (group)
                    mode |= 0o002  # Add write permission (others)
                    
                if "x" in permissions:
                    mode |= 0o100  # Add execute permission (user)
                    mode |= 0o010  # Add execute permission (group)
                ...
                message = "".join("Invalid permissions format. Please provide octal permissions like '777' or '644'")
        except PermissionError:
                message = "".join("Permission denied. You do not have permission to change the permissions of '{params}'.")
        return message
    return result
